...
void X12RtcFullResetClock(void){
    tm midnight = {0,0,0,1,0,100,0,0,0};
    X12RtcSetClock(&midnight);
}


void X12FillStringWithTime(char *stringToFillWithTime)
{
    tm gmt;
    X12RtcGetClock(&gmt);

    _StorableSetting storedHours;
    StorageLoadConfig(&storedHours); 

    char stringFilledWithTime[8] = "";
    sprintf(stringFilledWithTime, "%02d:%02d:%02d", (gmt.tm_hour+storedHours.timezone)%24, gmt.tm_min, gmt.tm_sec);
    strcpy(stringToFillWithTime,stringFilledWithTime);
}

int X12GetTimeZoneString(char *timeZoneString, int tempTimeZoneHours)
{
    printf("getTimeZoneString(%s , %d)\n",timeZoneString, tempTimeZoneHours);
    tm gmt;
    X12RtcGetClock(&gmt);
